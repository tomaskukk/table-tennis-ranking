// https://github.com/sc2iq/sc2iq/blob/master/packages/ratingSystem/src/index.ts
...
) => [playerAProbability: number, playerBProbability: number, ratingADifference: number, ratingBDifference: number];
...
function createNextRatingsFn(
  ...
  const nextRatingsFn = (playerARating: number, playerBRating: number, playerAScore: number) => {
    const [expectedPlayerAProbability, expectedPlayerBProbability] = getExpectedPlayerProbabilities(
      playerARating,
      playerBRating,
    );
...
export type KFactorOption = KFactorFunctionWithPlayers | KFactorFunction | number;

export function createRatingSystem(
  kFactor: KFactorOption = 32,
  exponentDenominator: number = 400,
